...
class _GroupModerationPageState extends State<GroupModerationPage> {
  ...
  Future<void> load() async {
    try {
      ...
      final ids = <String>{...ms.map((m) => '${m['user_id']}'), ...bans.map((b) => '${b['user_id']}')};
  ...
  Future<void> deleteMessage(String id) async {
    ...
    setState(() => busy = true);
    try {
      await db.rpc('delete_group_message_for_everyone', params: {'p_conversation_id': widget.conversationId, 'p_message_id': id});
      _toast('پیام برای همه حذف شد.');
      await load();
  ...
  Future<void> banMember(String id) async {
    if (!isAdmin || busy || id == '${group?['created_by']}') return;
    ...
    setState(() => busy = true);
    try {
      await db.rpc('ban_group_member', params: {'p_conversation_id': widget.conversationId, 'p_user_id': id, 'p_reason': text.isEmpty ? null : text});
      _toast('عضو حذف و محروم شد.');
      await load();
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: ListView(padding: const EdgeInsets.all(14), children: [
          ...
          Text('پیام‌های اخیر (${active.length})', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
          ...active.map((m) => Card(child: ListTile(
            ...
            trailing: IconButton(onPressed: busy ? null : () => deleteMessage('${m['id']}'), icon: const Icon(Icons.delete_sweep_rounded)),
          ))),
          ...
          ...members.map((m) {
            ...
            return Card(child: ListTile(
              ...
              subtitle: Text(owner ? '👑 مالک' : '${m['role'] ?? 'member'}'),
              trailing: owner || id == db.auth.currentUser?.id ? null : IconButton(onPressed: busy ? null : () => banMember(id), icon: const Icon(Icons.person_remove_rounded)),
            ));
          }),
          const SizedBox(height: 14),
          Text('محروم‌شده‌ها (${banned.length})', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
          ...banned.map((b) { final id = '${b['user_id']}'; return Card(child: ListTile(title: Text(nameOf(id)), subtitle: Text('${b['reason'] ?? 'بدون دلیل'}'), trailing: IconButton(onPressed: busy ? null : () => unbanMember(id), icon: const Icon(Icons.lock_open_rounded)))); }),
        ]),
      ),
    );
  }
