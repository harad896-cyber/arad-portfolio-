...
class _GroupManagementPageState extends State<GroupManagementPage> {
  ...
  Future<void> load() async {
    try {
      final g = Map<String, dynamic>.from(await db.from('conversations').select('id,type,title,avatar_url,description,created_by,allow_reactions,allow_member_add,is_public,username,auto_delete_seconds,join_approval,only_admins_can_post,only_admins_can_add').eq('id', widget.conversationId).single());
      ...
      setState(() {
        ...
        isAdmin = uid != null && (uid == g['created_by'].toString() || rows.any((m) => m['user_id'].toString() == uid && ['admin', 'owner'].contains(m['role'])));
        loading = false;
      });
    } catch (e) {
      if (mounted) { setState(() => loading = false); toast('خطا در بارگذاری گروه: $e'); }
    }
  }

  Future<void> updateSettings({String? title, String? description, bool? publicGroup, bool? approval, bool? adminsPost, bool? adminsAdd, bool? reactions, int? autoDelete}) async {
    ...
    try {
      await db.rpc('update_group_settings', params: {
        ...
        'p_title': title ?? g['title'], 'p_description': description ?? g['description'], 'p_avatar_url': g['avatar_url'],
        'p_is_public': publicGroup ?? g['is_public'] ?? false, 'p_username': g['username'],
        'p_join_approval': approval ?? g['join_approval'] ?? false, 'p_only_admins_can_post': adminsPost ?? g['only_admins_can_post'] ?? false,
        'p_only_admins_can_add': adminsAdd ?? g['only_admins_can_add'] ?? false, 'p_auto_delete_seconds': autoDelete ?? g['auto_delete_seconds'] ?? 0,
  ...
  Future<void> editGroup() async {
    ...
    await showDialog<void>(context: context, builder: (d) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [TextField(controller: title, decoration: const InputDecoration(labelText: 'نام گروه')), TextField(controller: desc, maxLines: 3, decoration: const InputDecoration(labelText: 'توضیحات'))]),
      actions: [TextButton(onPressed: () => Navigator.pop(d), child: const Text('لغو')), FilledButton(onPressed: () { Navigator.pop(d); updateSettings(title: title.text.trim(), description: desc.text.trim()); }, child: const Text('ذخیره'))],
    ));
  ...
  Future<void> removeMember(String id) async {
    ...
    try { await db.rpc('remove_group_member', params: {'p_conversationId': widget.conversationId, 'p_user_id': id}); await load(); toast('عضو حذف شد.'); }
    catch (e) { toast('حذف عضو ناموفق بود: $e'); }
  }

  Future<void> changeRole(String id, String role) async {
    if (!isAdmin || busy) return;
    try { await db.rpc('set_group_member_role', params: {'p_conversation_id': widget.conversationId, 'p_user_id': id, 'p_role': role}); await load(); }
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text((group!['title'] ?? widget.title).toString()), actions: [if (isAdmin) IconButton(onPressed: busy ? null : editGroup, icon: const Icon(Icons.edit_rounded))]),
      body: RefreshIndicator(
        onRefresh: load,
        child: ListView(padding: const EdgeInsets.all(14), children: [
          Card(child: ListTile(title: Text((group!['title'] ?? widget.title).toString(), style: const TextStyle(fontWeight: FontWeight.w900)), subtitle: Text('${members.length} عضو'))),
          if (isAdmin) ...[
            ...
            Card(child: ListTile(leading: const Icon(Icons.admin_panel_settings_rounded), title: const Text('مدیریت پیام و محرومیت'), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => GroupModerationPage(conversationId: widget.conversationId, title: (group!['title'] ?? widget.title).toString()))).then((_) => load()))),
          ],
