...
class _GroupManagementPageState extends State<GroupManagementPage> {
  ...
  Future<void> load() async {
    try {
      ...
      final ids = rows.map((e) => '${e['user_id']}').toList();
      ...
      final map = {for (final p in people) '${p['id']}': p};
      ...
      setState(() {
        ...
        members = rows;
        profiles = map;
        isAdmin = uid != null && (uid == '${g['created_by']}' || rows.any((m) => '${m['user_id']}' == uid && ['admin', 'owner'].contains(m['role'])));
        loading = false;
      });
    } catch (e) {
  ...
  Future<void> updateSettings({String? title, String? description, bool? publicGroup, bool? approval, bool? adminsPost, bool? adminsAdd, bool? reactions, int? autoDelete}) async {
    ...
    try {
      await db.rpc('update_group_settings', params: {
        ...
        'p_is_public': publicGroup ?? g['is_public'] ?? false,
        'p_username': g['username'],
  ...
  Future<void> removeMember(String id) async {
    ...
    try { await db.rpc('remove_group_member', params: {'p_conversationId': widget.conversationId, 'p_user_id': id}); await load(); _toast('عضو حذف شد.'); }
    catch (e) { _toast('حذف عضو ناموفق بود: $e'); }
  }
  ...
  Future<void> changeRole(String id, String role) async {
    ...
    catch (e) { _toast('تغییر نقش ناموفق بود: $e'); }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('${group!['title'] ?? widget.title}'), actions: [if (isAdmin) IconButton(onPressed: busy ? null : editGroup, icon: const Icon(Icons.edit_rounded))]),
      body: RefreshIndicator(
        onRefresh: load,
        child: ListView(padding: const EdgeInsets.all(14), children: [
          Card(child: ListTile(title: Text('${group!['title'] ?? widget.title}', style: const TextStyle(fontWeight: FontWeight.w900)), subtitle: Text('${members.length} عضو'))),
          if (isAdmin) ...[
            ...
            Card(child: ListTile(leading: const Icon(Icons.admin_panel_settings_rounded), title: const Text('مدیریت پیام و محرومیت'), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => GroupModerationPage(conversationId: widget.conversationId, title: '${group!['title'] ?? widget.title}'))).then((_) => load()))),
          ],
          const SizedBox(height: 8),
          Text('اعضا (${members.length})', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
          ...members.map((m) {
            final id = '${m['user_id']}';
            ...
            return Card(child: ListTile(
              ...
              trailing: isAdmin && !owner && id != db.auth.currentUser?.id ? PopupMenuButton<String>(onSelected: (v) => v == 'remove' ? removeMember(id) : changeRole(id, v), itemBuilder: (_) => [PopupMenuItem(value: role == 'admin' ? 'member' : 'admin', child: Text(role == 'admin' ? 'برداشتن مدیریت' : 'ارتقا به مدیر')), const PopupMenuItem(value: 'remove', child: Text('حذف از گروه'))]) : null,
            ));
          }),
        ],
      ),
