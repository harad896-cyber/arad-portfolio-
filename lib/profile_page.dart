...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> saveProfile() async {
    ...
    try{
      ...
      if(cleanUsername.isNotEmpty && !RegExp(r'^[a-z0-9_]{3,32}
...
).hasMatch(cleanUsername)){showMsg(context,'آیدی باید ۳ تا ۳۲ کاراکتر و فقط شامل حروف انگلیسی، عدد و _ باشد.');setState(()=>saving=false);return;}
      final data={'id':uid,'display_name':display,'username':cleanUsername,'bio':bio.text.trim(),'is_online':true,'last_seen':DateTime.now().toIso8601String()};
      await supabase.from('profiles').upsert(data); profile.addAll(data);
      if(mounted){setState((){});showMsg(context,'پروفایل ذخیره شد.');}
    }catch(e){if(mounted)showMsg(context,'ذخیره پروفایل ناموفق بود: $e');}
    finally{if(mounted)setState(()=>saving=false);}
