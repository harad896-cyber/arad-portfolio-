...
String _friendlyError(String message) {
  ...
  if (lower.contains('413') ||
    ...
    return 'این فایل خیلی بزرگ است.';
  }
  if (lower.contains('permission denied') ||
      lower.contains('not authorized') ||
      lower.contains('unauthorized')) {
    return 'دسترسی لازم برای انجام این کار را ندارید.';
  }
                ...
                child: offline
                    ? Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            Icon(Icons.cloud_off_rounded, size: 17, color: Theme.of(context).colorScheme.error),
                            const SizedBox(width: 8),
                            Text('در حال اتصال...', style: TextStyle(fontWeight: FontWeight.w700, color: Theme.of(context).colorScheme.onSurface)),
                          ],
                        ),
                      )
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          TextField(controller: username, onChanged: (v) => setState(() => usernameError = v.trim().replaceFirst('@','').isEmpty || RegExp(r'^[A-Za-z0-9_]{3,32} prefixText: '@', border: OutlineInputBorder())),
          const SizedBox(height: 12),
...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> _loadOlderMessages() async {
    ...
    try {
      ...
      older.sort((a, b) => (DateTime.tryParse('${a['created_at']}') ?? DateTime.fromMillisecondsSinceEpoch(0))
          .compareTo(DateTime.tryParse('${b['created_at']}') ?? DateTime.fromMillisecondsSinceEpoch(0)));
      _hasOlderMessages = older.length == _messagePageSize;
      if (older.isEmpty) return;
      final before = _messagesScroll.hasClients ? _messagesScroll.position.maxScrollExtent : 0.0;
      await _hydrateMessages(older);
      if (!mounted) return;
      setState(() => messages = [...older, ...messages]);
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted || !_messagesScroll.hasClients) return;
        final delta = _messagesScroll.position.maxScrollExtent - before;
                  ...
                  : messages.isEmpty
                      ...
                      : NotificationListener<ScrollNotification>(
                          onNotification: (notification) {
                            if (notification.metrics.pixels <= notification.metrics.minScrollExtent + 240) {
                              _loadOlderMessages();
                            }
                            return false;
                          },
                            ...
                            itemBuilder: (context, i) {
                              if (_loadingOlder && i == 0) {
                                return const Padding(
                                  ...
                                  child: SizedBox(width: 24, height: 24, child: Center(child: CircularProgressIndicator(strokeWidth: 2))),
                                );
                              }
                              final messageIndex = i - (_loadingOlder ? 1 : 0);
                              return _glassMessageBubble(messages[messageIndex]);
                            },
                          ),
                        ),
            ),
...
class _ChatPageState extends State<ChatPage> {
  ...
  }}).hasMatch(v.trim().replaceFirst('@','')) ? null : 'نام کاربری نامعتبر است.'), decoration: InputDecoration(errorText: usernameError, labelText: 'نام کاربری', prefixText: '@', border: OutlineInputBorder())),
          const SizedBox(height: 12),
          TextField(controller: bio, maxLines: 3, decoration: const InputDecoration(labelText: 'معرفی کوتاه', border: OutlineInputBorder())),
          const SizedBox(height: 16),
          FilledButton(onPressed: busy ? null : save, child: Text(busy ? 'در حال ذخیره...' : 'ادامه')),
