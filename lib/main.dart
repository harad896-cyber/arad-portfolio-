...
class _HomePageState extends State<HomePage> {
  ...
  List<Map<String, dynamic>> get visibleChats {
    ...
    return chats.where((c) {
      ...
      final matchesFolder = folderSet == null || folderSet.contains(c['id'].toString());
  ...
  Future<void> load() async {
    try {
      ...
      final assignmentMap = <String, Set<String>>{};
      for (final folder in folderRows) {
        assignmentMap[folder['id'].toString()] = <String>{};
      }
      if (folderRows.isNotEmpty) {
        ...
        for (final item in itemRows) {
          (assignmentMap[item['folder_id'].toString()] ??= <String>{}).add(item['conversation_id'].toString());
        }
      }
      if (mounted) setState(() {
        chats = enriched;
  ...
  Future<void> _openFolderManager() async {
    await Navigator.push(context, MaterialPageRoute(builder: (_) => const ChatFolderManagerPage()));
    if (mounted) {
      setState(() {
