  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: loading ? const Center(child: CircularProgressIndicator()) : rows.isEmpty
      ...
      : RefreshIndicator(onRefresh: load, child: ListView.separated(
          ...
          itemBuilder: (_, i) { final r = rows[i]; final type = '${r['type']}'; final title = '${r['title'] ?? (type == 'group' ? 'گروه' : type == 'channel' ? 'کانال' : 'گفتگو')}'; return Card(child: ListTile(
            leading: CircleAvatar(child: Icon(type == 'group' ? Icons.group : type == 'channel' ? Icons.campaign : Icons.person)),
            title: Text(title, style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text('${r['unread_count']} پیام خوانده‌نشده'),
...
class _ConversationStatsPageState extends State<ConversationStatsPage> {
  bool loading = true; List<Map<String, dynamic>> stats = [];
  Future<void> load() async {
    stats = [];
    try {
      final db = Supabase.instance.client;
      final raw = await db.rpc('get_conversation_sender_stats', params: {'p_conversation_id': widget.conversationId});
      final base = List<Map<String, dynamic>>.from(raw as List); final ids = base.map((e) => '${e['sender_id']}').toList(); final result = <Map<String, dynamic>>[];
      if (ids.isNotEmpty) {
    ...
    } catch (e) { if (mounted) { setState(() => loading = false); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('آمار پیام‌ها بارگذاری نشد: $e'))); } }
  }
  @override void initState() { super.initState(); load(); }
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: loading ? const Center(child: CircularProgressIndicator()) : stats.isEmpty ? const Center(child: Text('هنوز پیامی ارسال نشده است.')) : ListView.builder(
      ...
      itemBuilder: (_, i) { final s = stats[i]; final p = s['profile'] is Map ? Map<String, dynamic>.from(s['profile']) : <String, dynamic>{}; final avatar = '${p['avatar_url'] ?? ''}'; return ListTile(
        ...
        title: Text('${p['display_name'] ?? p['username'] ?? 'کاربر'}', style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text('@${p['username'] ?? ''}'), trailing: Text('${s['message_count']} پیام'),
      ); },
    ),
  );
}
