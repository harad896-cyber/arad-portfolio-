...
# Clipboard is used by the message action menu.
...
# Remove an unused import that was only needed by an earlier patch.
s = s.replace("import 'group_management.dart';\n", "")

# Work only inside ChatPage so similarly named declarations elsewhere are untouched.
a = s.index('class ChatPage extends StatefulWidget {')
b = s.index('class ProfilePage extends StatefulWidget {', a)
chat = s[a:b]

# Keep exactly one reactions state declaration.
...
# Repair the invalid Wrap(padding: ...) expression produced by the message action patch.
chat = chat.replace(
...
# Close the extra Padding introduced above at the end of the reaction picker.
chat = chat.replace(
    ").toList()))); }\n  Future<void> actions",
    ").toList())))); }\n  Future<void> actions"
)

# Replace only the ChatPage build method with a conservative, fully formatted implementation.
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            CircleAvatar(
              radius: 19,
              child: Text(widget.title.isEmpty ? 'A' : widget.title.substring(0, 1)),
            ),
        ...
        actions: [
          IconButton(
            onPressed: () => showMsg(context, 'جستجوی پیام‌ها به‌زودی فعال می‌شود.'),
                            ...
                            child: Align(
                              ...
                              child: Container(
                                ...
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    if (m['reply_to'] != null && !deleted)
                                      const Text('↪ پاسخ به پیام', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700)),
                                    if (m['message_type'] == 'image' && !deleted)
                                      const Padding(padding: EdgeInsets.only(bottom: 4), child: Icon(Icons.image_rounded)),
                                    if (m['message_type'] == 'file' && !deleted)
                                      const Padding(padding: EdgeInsets.only(bottom: 4), child: Icon(Icons.insert_drive_file_rounded)),
                                    Text(
                                      deleted ? 'این پیام حذف شده است' : body,
                                      style: TextStyle(fontSize: 15.5, height: 1.35, fontStyle: deleted ? FontStyle.italic : FontStyle.normal),
                                    ),
                                    if (m['edited_at'] != null && !deleted)
                                      const Padding(padding: EdgeInsets.only(top: 3), child: Text('ویرایش‌شده', style: TextStyle(fontSize: 9))),
                                    if (messageReactions.isNotEmpty && !deleted)
                                      Padding(
                                        padding: const EdgeInsets.only(top: 4),
                                        child: Text(messageReactions.take(6).join(' '), style: const TextStyle(fontSize: 16)),
                                      ),
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        if (mine) ...[
                                          ...
                                          Icon(m['read_at'] != null ? Icons.done_all_rounded : Icons.done_rounded, size: 15),
                                        ],
                                      ],
                                    ),
                                  ],
          ...
          SafeArea(
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  IconButton(
                    onPressed: messages.isEmpty ? null : () => pickReaction(messages.last),
