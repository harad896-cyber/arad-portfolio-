    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          PopupMenuButton<String>(itemBuilder: (_) => const [PopupMenuItem(value: 'manage', child: Text('مدیریت گفتگو')), PopupMenuItem(value: 'mute', child: Text('بی‌صدا کردن'))], onSelected: (v) => showMsg(context, v == 'mute' ? 'تنظیم بی‌صدا کردن آماده است.' : 'مدیریت گفتگو')),
        ],
      ),
      body: Column(children: [
        Expanded(child: loading ? const Center(child: CircularProgressIndicator()) : messages.isEmpty
          ? const Center(child: Text('هنوز پیامی نیست', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800)))
          : ListView.builder(padding: const EdgeInsets.fromLTRB(12, 14, 12, 18), itemCount: messages.length, itemBuilder: (context, index) {
              final m = messages[index]; final mine = m['sender_id'] == uid; final deleted = m['deleted_at'] != null; final body = '${m['body'] ?? ''}';
              final messageReactions = reactions['${m['id']}'] ?? <String>[]; final created = DateTime.tryParse('${m['created_at'] ?? ''}');
              final time = created == null ? '' : '${created.hour.toString().padLeft(2, '0')}:${created.minute.toString().padLeft(2, '0')}';
              return GestureDetector(onLongPress: () => actions(m), child: Align(alignment: mine ? Alignment.centerRight : Alignment.centerLeft, child: Container(
                constraints: const BoxConstraints(maxWidth: 350), margin: const EdgeInsets.symmetric(vertical: 4), padding: const EdgeInsets.fromLTRB(13, 9, 10, 7),
                decoration: BoxDecoration(color: mine ? theme.colorScheme.primaryContainer : theme.colorScheme.surfaceContainerHighest, borderRadius: BorderRadius.only(topLeft: const Radius.circular(20), topRight: const Radius.circular(20), bottomLeft: Radius.circular(mine ? 20 : 5), bottomRight: Radius.circular(mine ? 5 : 20))),
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Row(mainAxisSize: MainAxisSize.min, children: [Text(time, style: const TextStyle(fontSize: 10)), if (mine) ...[const SizedBox(width: 4), Icon(m['read_at'] != null ? Icons.done_all_rounded : Icons.done_rounded, size: 15)]]),
                ]),
              )));
            })),
        if (replyToId != null) Container(margin: const EdgeInsets.fromLTRB(10, 0, 10, 5), padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: theme.colorScheme.secondaryContainer, borderRadius: BorderRadius.circular(14)), child: Row(children: [const Icon(Icons.reply, size: 18), const SizedBox(width: 7), Expanded(child: Text(replyToText ?? 'پاسخ', maxLines: 1, overflow: TextOverflow.ellipsis)), IconButton(onPressed: () => setState(() { replyToId = null; replyToText = null; }), icon: const Icon(Icons.close, size: 18))])),
        SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(8, 5, 8, 8), child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
          IconButton(onPressed: messages.isEmpty ? null : () => pickReaction(messages.last), icon: const Icon(Icons.add_reaction_outlined)),
          IconButton(onPressed: sendImage, icon: const Icon(Icons.photo_outlined)), IconButton(onPressed: sendFile, icon: const Icon(Icons.attach_file_rounded)),
          Expanded(child: TextField(controller: text, minLines: 1, maxLines: 5, decoration: InputDecoration(hintText: 'پیام...', filled: true, fillColor: theme.colorScheme.surfaceContainerHighest, border: OutlineInputBorder(borderRadius: BorderRadius.circular(22), borderSide: BorderSide.none), contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 11)))),
          const SizedBox(width: 4), IconButton.filled(onPressed: sending ? null : sendText, icon: const Icon(Icons.send_rounded)),
        ]))),
      ]),
    );
  }
