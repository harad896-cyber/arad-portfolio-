...
REPO_CLEAN = '6e215c34a2bc44bdab38462b53528522b43362cf'
path = Path('lib/main.dart')
text = path.read_text(encoding='utf-8')

# Always start from the last verified full source so a broken placeholder cannot survive.
if 'intentionally replaced in the repository' in text or 'class ProfilePage extends StatefulWidget' not in text:
    subprocess.run(['git', 'checkout', REPO_CLEAN, '--', 'lib/main.dart'], check=True)
    text = path.read_text(encoding='utf-8')

if "import 'invite.dart';" not in text:
    marker = "import 'package:supabase_flutter/supabase_flutter.dart';\n"
    text = text.replace(marker, marker + "import 'invite.dart';\n", 1)

settings = """          const SizedBox(height: 22),
          const Divider(),
...
anchor = "          FilledButton.tonal(onPressed: () => supabase.auth.signOut(), child: const Text('خروج')) ,"
anchor = anchor.replace("'خروج')) ,", "'خروج')),")
if 'تنظیمات پروفایل' not in text:
    if anchor not in text:
        raise SystemExit('ProfilePage anchor not found')
    text = text.replace(anchor, settings + "\n" + anchor, 1)

if 'class ProfileOptionPage extends StatelessWidget' not in text:
    text += """
...
if subprocess.run(['git', 'diff', '--cached', '--quiet']).returncode != 0:
    subprocess.run(['git', 'commit', '-m', 'feat: move selected settings into profile'], check=True)
