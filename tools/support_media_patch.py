...
# Insert attachment loading immediately after message ids are declared.
anchor="""      final ids = loaded.map((m) => '${m['id']}').toList();
"""
insert="""      final ids = loaded.map((m) => '${m['id']}').toList();
      attachmentUrls = {};
      if (ids.isNotEmpty) {
        try {
...
"""
if extra not in s and needle in s:
