...
CLEAN = 'f38f2c2ba8a83fe4064b874f0c2049a1e0afdf97'
p = Path('lib/main.dart')
subprocess.run(['git', 'checkout', CLEAN, '--', 'lib/main.dart'], check=True)
s = p.read_text(encoding='utf-8')

# OTP: six digits, explicit button verification only.
s = s.replace("hintText: '۱۲۳۴۵۶'", "hintText: '------'")
s = s.replace("hintText: '123456'", "hintText: '------'")
s = re.sub(r"\n\s*if \(normalized\.length == 6 && !busy && !_verificationStarted\) \{\n\s*verify\(\);\n\s*\}", '', s)
s = re.sub(r"\n\s*onSubmitted: \(_\) \{\n\s*if \(!busy\) verify\(\);\n\s*\},", '', s)
s = s.replace('بعد از وارد کردن رقم ششم، کد خودکار بررسی می‌شود و در صورت درست بودن وارد برنامه می‌شوید.', 'کد فقط وقتی بررسی می‌شود که روی «تأیید و ورود» بزنید. کدهای قبلی یا منقضی‌شده قابل استفاده نیستند.')
s = s.replace('پس از ارسال، یک کد ۶ رقمی به ایمیل شما می‌آید و بعد از رقم ششم خودکار بررسی می‌شود.', 'پس از ارسال، یک کد دقیقاً ۶ رقمی به ایمیل شما می‌آید. کد فقط با زدن «تأیید و ورود» بررسی می‌شود.')

if "import 'dart:typed_data';" not in s:
    s = s.replace("import 'dart:async';", "import 'dart:async';\nimport 'dart:typed_data';", 1)

PROFILE = r'''class ProfileSetupPage extends StatefulWidget {
...
HOME = r'''class HomePage extends StatefulWidget {
...
s = re.sub(r'class ProfileSetupPage extends StatefulWidget \{.*?\nclass HomePage extends StatefulWidget \{', PROFILE + '\nclass HomePage extends StatefulWidget {', s, count=1, flags=re.S)
s = re.sub(r'class HomePage extends StatefulWidget \{.*?\nclass UserSearchDelegate extends SearchDelegate', HOME + '\nclass UserSearchDelegate extends SearchDelegate', s, count=1, flags=re.S)

# Group creation: deduplicate member IDs so creator cannot be inserted twice.
s = s.replace("""      final members = <Map<String, dynamic>>[\n        {'conversation_id': c['id'], 'user_id': uid},\n        ...selected.map((p) => {'conversation_id': c['id'], 'user_id': p['id']}),\n      ];""", """      final uniqueIds = <String>{uid, ...selected.map((p) => '${p['id']}')};\n      final members = uniqueIds.map((memberId) => {'conversation_id': c['id'], 'user_id': memberId}).toList();""", 1)

p.write_text(s, encoding='utf-8')
print('clean source recovered and patched')
